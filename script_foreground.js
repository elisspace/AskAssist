...
window.addEventListener('load', function () {
    ...
    let attempts = 0;
    let workspaceCheck = this.setInterval(function () {
        console.log('looking for workspace')
        let workspace = document.querySelector('.p-workspace__primary_view_contents');
        if (workspace) {
            console.log(`workspace loaded, appending tool on attempt #${attempts}`)
            appendTool();
            attempts++;
            clearInterval(workspaceCheck);
        } else if (attempts > 20) {
            alert('AskAssist cannot append to .p-workspace__primary_view_contents; element not found. Please poke Jeremy Dixon if you dont know why :) ');
            clearInterval(workspaceCheck);
        } else {
